...
import { encodeInput, decodeOutcome, encodeResult, decodeRequest } from "../../src/embedding/index.mjs";
...
let boundaries = 0;
try {
  for (const name of ["retainedScope", "retainedScopeGeneral", "branchingTailProtected", "branchingTail", "resource", "generator", "reentrant", "custody", "components", "componentsDouble"]) {
    ...
    const quantum = name.startsWith("branchingTail") || name.startsWith("retainedScope") ? 1 : 17;
    ...
    for (let round = 0; ; round++) {
      ...
      if (["completed", "failed", "cancelled"].includes(outcome.kind)) {
        if (name.startsWith("retainedScope")) {
          assert.equal(outcome.kind, "completed");
          assert.deepEqual(outcome.value, Uint8Array.of(0x61, 4, 0, 0, 0, 0, 0, 0, 99, 0, 0, 0, 0, 0, 0, 0));
        }
      ...
      if (outcome.kind === "requested") {
        ...
        assert.ok(["retained-scope/release", "example/tail-cleanup", "example/resource-acquire", "example/resource-use", "example/resource-release", "example/generator-release", "custody/release", "component/release"].includes(request.semanticIdentity));
        if (request.semanticIdentity === "retained-scope/release") assert.deepEqual(request.payload, Uint8Array.of(77, 0, 0, 0, 0, 0, 0, 0));
