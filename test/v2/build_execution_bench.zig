...
pub fn build(b: *std.Build) void {
    ...
    const root = b.createModule(.{ .root_source_file = b.path("execution_bench.zig"), .target = b.graph.host, .optimize = .ReleaseSafe, .imports = &.{ .{ .name = "boundary", .module = boundary }, .{ .name = "world", .module = world } } });
