...
fn program(b: *source.Builder) !source.Module {
    ...
    try b.define(clause, try b.term(.{ .conditional = .{
        ...
        .when_false = try update(b, handler, resumption, try b.constant(bool, false)),
    } }));
    return application(b, boolean, values, operation, capability, handler);
}

fn update(b: *source.Builder, handler: u64, token: u64, consent: u64) !u64 {
...
fn application(
    ...
    try b.define(installed, try b.term(.{ .handle = .{
        ...
        .arguments = &arguments,
        .state = &.{try b.constant(bool, true)},
    } }));
...
fn reverseAnswer(
    ...
    for (&fields, 0..) |*field, index| {
        const consent = try b.primitive(boolean, .field, &.{answer}, 3 - index);
        field.* = try b.primitive(boolean, .equal, &.{
            consent, try b.constant(bool, false),
        }, 0);
    }
