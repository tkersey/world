...
pub fn build(b: *std.Build) void {
    ...
    const compiler = b.addExecutable(.{ .name = "compile-thermostat", .root_module = root });
    b.step("emit", "Compile the thermostat application into portable data").dependOn(&b.addRunArtifact(compiler).step);
}
