...
pub const OneEffectApp = world.application(.{
    .name = "one-effect",
    .version = "2.0.0",
    .root = RootMachine,
    .limits = application_limits,
    .external = .{world.external(RootMachine, 0, .{
...
pub const ProviderApp = world.application(.{
    ...
    .external = .{world.external(ProviderMachine, 0, .{
        ...
        .authority_requirements = @as(u64, 1) << @intFromEnum(world.Authority.file_read),
    })},
});

pub const DeferredApp = world.application(.{
    ...
    .external = .{world.external(RootMachine, 0, .{
        ...
        .allowed_statuses = world.protocol.v1.AllowedStatuses{ .deferred = true },
    })},
});

pub const TightResultApp = world.application(.{
    .name = "tight-result",
    .version = "2.0.0",
    .root = RootMachine,
    .limits = application_limits,
...
const RejectedApp = world.application(.{
...
const InvalidInputApp = world.application(.{
...
fn okResult(
    ...
    request: world.protocol.v1.EffectRequest,
    value: anytype,
) !world.protocol.v1.EffectResult {
...
test "site ids bind the owning Machine occurrence" {
    ...
    try std.testing.expect(
        world.siteId(RootMachine, 0) !=
            world.siteId(DuplicateRootMachine, 0),
    );
}
...
test "World Frame preserves distinct authored Machine failure tags" {
    ...
    try std.testing.expectEqual(world.protocol.v1.FrameStatus.failed, rejected.status);
    try std.testing.expectEqual(world.protocol.v1.FrameStatus.failed, invalid.status);
...
test "World closes one Boundary Machine ABI v2 residual effect" {
    ...
    try std.testing.expectEqual(world.siteId(RootMachine, 0), OneEffectApp.residual_effect_row[0].site_id);
    try std.testing.expectEqual(
        @as(u64, 1) << @intFromEnum(world.Authority.model),
        OneEffectApp.Manifest.required_host_capabilities,
    );
    try std.testing.expectEqualStrings("1.5.0", OneEffectApp.Manifest.boundary_package_version);
    try std.testing.expectEqualStrings("3.1.3", OneEffectApp.Manifest.world_package_version);
...
test "World delegates portable schema identity to Boundary" {
    try std.testing.expectEqualSlices(
        ...
        &world.valueSchemaId(SumA),
    );
    try std.testing.expect(!std.mem.eql(
        u8,
        &world.valueSchemaId(SumA),
        &world.valueSchemaId(SumB),
    ));
}
...
test "World deferred result preserves the exact outstanding request and Frame" {
    ...
    var deferred: world.protocol.v1.EffectResult = .{
...
test "World application executes one external effect deterministically" {
    ...
    try std.testing.expectEqual(world.protocol.v1.FrameStatus.completed, first.status);
...
test "World provider parks externally and resumes its exact parent" {
    ...
    try std.testing.expectEqual(world.siteId(ProviderMachine, 0), ProviderApp.residual_effect_row[0].site_id);

    const args = try ProviderApp.encodeInitialArgs(allocator, @as(u32, 9));
    const parent = try ProviderApp.initialFrame(&arena, args, 100);
    try std.testing.expectEqual(world.protocol.v1.FrameStatus.needs_effect, parent.status);
    try std.testing.expectEqual(world.siteId(ProviderMachine, 0), parent.pending_effect.?.site_id);
