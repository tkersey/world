...
const w=await verifyAssets(worldAssets,['world-process-kernel-v2.wasm','world-v5.0.2-process-runtime.tar.gz','world-v2-conformance.json','world-v2-conformance.bin','world-v5.0.2-release-receipt.json']);
const br=JSON.parse(b.get('boundary-v2-release-receipt.json')),wr=JSON.parse(w.get('world-v5.0.2-release-receipt.json'));
...
const runtimeEntries=readTarGzip(w.get('world-v5.0.2-process-runtime.tar.gz')),runtimeMap=new Map(runtimeEntries.map((entry)=>[entry.name,entry.bytes]));
