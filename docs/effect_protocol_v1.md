...
The Zig semantic record types borrow their slices and expose no owning deinitializer. Each `decode` operation instead returns `Decoded(T)`: its `value` is the validated record, and its `deinit` method releases only the storage allocated by that decode. Copying or directly constructing a semantic record never creates ownership and therefore cannot acquire a destructive cleanup operation.
