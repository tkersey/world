...
The universal WASM exposes ABI v2 manifest, load/unload, command submit, output, last-error, reset, memory, and bounded allocation exports. It has zero imports, no WASI, no filesystem, no network, no clock, no randomness, no host effect callback, no storage callback, and bounded linear memory.

Host exchange is canonical: HostRequests carry request/payload/idempotency/Actuation evidence bytes, and completed TurnOutputs carry root result, result refs, finalized receipts, RunReceipt, checkpoint, Capsule evidence, and Archive.AppendBatch bytes. Batched turns preserve missing and nonterminal requests, canonicalize reply order, reject duplicate or stale replies, and stop at the next quiescent boundary.
...
- `zig build check-world-v0` is the completion gate over Runtime Closure, executable image validation, World Seed examples, malformed rejection, and WASM inspection.
...
World v0 completion means: World can execute, suspend, migrate, replay, and audit a sealed Certified Boundary program closure in one generic native or WASM runtime.
