...
- Locked the active-Fabric rule: only parked or terminal active Fabric invocations with parent pending witness, provider witness, route witness, value mapping witness, and mailbox ownership can freeze/thaw; every other active route fails closed.

## Summary
Add World Assembly Capsule Kernel. A Capsule freezes a quiescent linked Runspace assembly into a deterministic portable image that captures Runspace slots, pending mailbox entries, active and completed Fabric causality, Linker/Assembly provenance, admission/environment/supervision evidence, transcripts, run images, value images, and guest conformance references. A receiver can validate, inspect, replay, relink, verify, restore parked/completed/failed states, reissue stricter local permits, remap run handles/mailbox ids, and resume supported parked assemblies without serializing handlers, credentials, native pointers, request tokens, or transport/storage state.
...
7. Capsule v1 is fail-closed for unsupported running state, unsafe active Fabric, missing witnesses, relink mismatch, environment mismatch, and permit denial.
...
7. Commit restores supported run images, parked/completed/failed slots, pending mailbox entries, completed receipts, active parked Fabric invocations, timeline/provenance refs, and restore receipt references.
...
- Freeze/Thaw: freezes completed linked assemblies, parked runs, and active parked Fabric; rejects unsafe active Fabric and non-quiescent runspace; inspect-only, replay-only, completed, parked, failed, active Fabric, verify, and relink restore modes behave according to policy.
...
- Agent/examples: linked restore, active Fabric restore, agent transfer, relink mismatch, guest verify, and supervised restore examples run with stable output.
...
- Abort if active Fabric restore cannot prove parent pending witness, provider witness, route witness, value mapping witness, supervision evidence, parent response mapping, and mailbox ownership before mutation.
...
5. step=thaw-relink-transaction; owner=implementation; success_criteria=validate/planThaw/thawIntoRunspace/verifyLink/relink support inspect/replay/completed/parked/failed/active-fabric/verify modes, local target/module matching, environment preflight, receiver permit issuance, handle/mailbox/Fabric remaps, deny-before-mutation, and focused thaw/relink/restore tests pass.
...
7. step=examples-docs-build; owner=implementation; success_criteria=six capsule examples, build run steps, README update, `docs/capsules.md`, and check-step wiring compile and demonstrate linked restore, active Fabric restore, agent transfer, relink mismatch, guest verify, and supervised restore.
