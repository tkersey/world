...
export const PROCESS_KERNEL_EXPORT_SIGNATURES = Object.freeze({
  world_process_v2_abi_version: signature([], ["i32"]),
  world_process_v2_prepare_input: signature(["i64"], ["i32"]),
  world_process_v2_input_ptr: signature([], ["i32"]),
  world_process_v2_input_capacity: signature([], ["i64"]),
  world_process_v2_execute: signature(["i64"], ["i32"]),
  world_process_v2_output_ptr: signature([], ["i32"]),
  world_process_v2_output_len: signature([], ["i64"]),
  world_process_v2_error_ptr: signature([], ["i32"]),
  world_process_v2_error_len: signature([], ["i64"]),
});
...
function inspectValidated(bytes) {
  ...
  return deepFreeze({
    format: "world-process-kernel-inspection/v2",
    wasm32: true,
    importCount,
...
function requireExports(exports, functions, types) {
  if (exports.size !== PROCESS_KERNEL_EXPORT_NAMES.length) {
    throw profileError("Process kernel export set does not match ABI v2", {
      exportCount: exports.size,
  ...
  for (const name of PROCESS_KERNEL_EXPORT_NAMES) {
    if (!exports.has(name)) {
      throw profileError("Process kernel is missing a required ABI v2 export", {
        exportName: name,
      });
    }
  }
